...
| **M4** | プロダクトバックログが運用できる | B-15 〜 B-20 | 🟨 5 / 6（B-15・B-16・B-17・B-18・B-20 完了・B-19 未着手。E2E①の緑化は横断課題として EX-1 に集約） |
...
| **EX** | 横断（着手順の外）— E2E 実行整備 等 | EX-1 | 0 / 1 |
| | | **合計** | **18 / 33（＋EX 0 / 1）** |
...
- [ ] **ヘッドレスのサインイン経路**を決めて実装する（テスト用トークン注入 or 開発用サインイン省略 B-14。CI で対話サインインなしに通る。方式を `docs/decisions` に記録）
- [ ] `playwright.config.ts` の `webServer` を有効化し、app を起動して E2E を回す導線を通す
- [ ] **既知の初期状態**を seeding で毎回作り直せる（E2E は初期状態から。D-19）
- [ ] **E2E フロー①を CI で緑にする**（`frontend/e2e/signin-pbi-task.spec.ts`。本体は B-20 実装済み・対象画面は B-17/B-20 で存在）
- [ ] 残りフロー②〜⑤は対象画面が揃い次第、本PBIのサインイン経路を使って各担当PBI（B-22/B-23/B-25/B-29）で `test.fixme` を外す（本PBIは共通基盤を提供し続ける）
- [ ] E2E は **毎 PR（マージ前ゲート）**で回す（main への push でデプロイするため、マージ後では壊れた本番を作ってから気づくことになりゲートにならない — D-19 改定・`e2e.yml`）
...
| **EX-1** | 🟡 横断/番号外 | E2E を CI で実際に緑にする実行面の整備（サインイン経路・`webServer`・seeding）。5本共通の土台でどの機能PBIにも属さないため、着手順の B-NN とは別枠に切り出した |
...
横断課題として **EX-1 に集約**（機能スコープは完結）。あわせて同じ理由で保留だった B-17 も完了に。
**E2E の実行整備を EX-1（横断・番号外の新設PBI）に切り出したことで、B-11（テスト戦略とハーネス
の確立）も完了**にできた。M3 は 3/4（残り B-14 任意）、M4 は 5/6（残り B-19）。B-series 18/33＋EX 0/1）_
