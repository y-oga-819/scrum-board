...
_最終更新: 2026-08-06（**M5 達成**。B-25 スプリント終了処理を完了し、B-26 楽観排他 UX を証跡付きで
閉じた。B-25 は着手前に持ち越しの規則・移動先・締められる条件を
[`D-26`](./decisions/D-26-sprint-close.md) で決めた——**未完了だけを指定した次スプリントへ持ち越し、
完了は凍結**（I-5・単一文書で表せない操作級の不変条件を終了操作に閉じる）。`close/preview`（読み取り）と
`close`（確定）を分け（D-20・P-1）、締められるのは `active` だけ、`If-Match` を取らないサーバー所有操作。
ボードに開始→終了の導線を足し、M5 の「1 周」を一筆で辿れるようにした。B-26 は方針 D-24（再操作を促す・
自動マージ却下・412 本文に最新値を含めない）が既決で、実装は B-23（ボード）＋横断適用済み。版を持つ
全書き込み経路（board/backlog/pbi-detail）が 412 時に**黙って上書きせず再取得**することを監査し、
lost update の経路が無いことを確認して閉じた。M3 は 3/4（残り B-14 任意）、M4・M5 は達成。次は M6
（デイリースクラム）。B-series 25/33＋EX 2/2）_
