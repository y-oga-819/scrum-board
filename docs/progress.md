...
| **EX** | 横断（着手順の外）— E2E 実行整備 等 | EX-1 | ✅ 1 / 1 |
| | | **合計** | **18 / 33（＋EX 1 / 1）** |
...
_最終更新: 2026-08-02（EX-1 E2E 実行整備を完了。実サーバ+実ブラウザでは `dependency_overrides`
が効かないため、サインインをバックエンドの env ゲート resolver（既定 OFF・fail-closed）と
フロントの e2e ビルド（MSAL 無効化）でバイパスする方式を [`D-22`](./decisions/D-22-e2e-execution.md)
に決定。Cosmos エミュレータへマイグレーション全適用のうえ `prd_test_<runId>` を seeding し
（`prd_test_` 以外は拒否・teardown は物理削除）、`webServer` で e2e SPA + FastAPI を起動して
フロー①（`signin-pbi-task.spec.ts`）を CI で緑にした。②〜⑤はこの共通基盤の上で各担当PBIが
`test.fixme` を外す。あわせて古い「main マージ時」の記述を「毎 PR（マージ前ゲート）」に整合。
M3 は 3/4（残り B-14 任意）、M4 は 5/6（残り B-19）。B-series 18/33＋EX 1/1）_
