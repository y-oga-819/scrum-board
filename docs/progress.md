...
| **M5** | ★**スプリントが1周回る**（ここからドッグフーディング） | B-21 〜 B-26 | 🟨 2 / 6（B-21・B-22 完了） |
...
_最終更新: 2026-08-06（B-22 プランニングモード（右ペイン）を完了。「どの PBI を今スプリントで
回すか」を、**配下タスクの `sprintId` の付け外し**として実装した——PBI 自身はスプリントへの参照を
持たず（D-08）、「今スプリントにいる」は配下タスクから導出する（二重に持たない）。規則は専用
エンドポイント `POST/DELETE .../sprints/{sid}/pbis/{pbiId}`（`app/api/planning.py`）と純関数
`app/data/planning.py` に閉じ、取り込みは未完了タスクに `sprintId` を付け**タスク0件なら「タスク
分解」を1件生成**（D-15）、外すは未完了だけを戻し**完了タスクは動かさない**（I-5）。フロントは画面A
にプランニング右ペインを足し、チェック状態は導出（保持しない）。E2E フロー②の `test.fixme` を外した。
直前は B-21 スプリントの CRUD を完了し **M5 に着手**（ドメイン規則を純関数 `app/data/sprints.py` に
一元化・`number` の連番採番）。M3 は 3/4（残り B-14 任意）、M4 は 6/6 達成、M5 は 2/6（B-21・B-22
完了・次は B-23 スプリント画面のボード。着手前に Q-F＝412 発生後の UX を決める）。
B-series 21/33＋EX 2/2）_
