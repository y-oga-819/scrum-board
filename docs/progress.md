...
> **画面と集約 GET は実装完了。残るは E2E フロー①の緑化のみで、これは仕様どおり
> B-20（タスク追加UI）待ち（2026-08-01）。** 読み取りを**画面単位**にし
...
- [ ] **B-11 の E2E フロー①（サインイン→PBI作成→タスク追加, `frontend/e2e/signin-pbi-task.spec.ts`）の `test.fixme` を外して緑にする**（D-19 主要フロー網羅）　→ **タスク追加UIが B-20 に載るため、B-18/B-20 が揃う時点で緑化する**（この条件は本 PBI の完了条件で明示的にその繰り延べを許している。フロー本体の「PBI 作成」導線は本 PBI で用意済み＝`PBI を追加`／`タイトル`／`保存`）
