...
| **M5** | ★**スプリントが1周回る**（ここからドッグフーディング） | B-21 〜 B-26 | 🟨 3 / 6（B-21・B-22・B-23 完了） |
...
_最終更新: 2026-08-06（B-23 スプリント画面のボードを完了。着手前に **Q-F＝412 発生後の UX** を
[`D-24`](./decisions/D-24-optimistic-concurrency-ux.md) で決めた——**再操作を促す＋即時再取得**
（自動マージは却下・412 本文に最新値を含めない。P-1）。ボード（画面B・`/board`）は選んだスプリントの
タスクを **todo/doing/done** の 3 カラムに並べ、ドラッグで status を移し isBlocked を立てる。要点は
**書き込みを新設しない**こと——移動もブロックも B-20 の汎用 `PATCH /tasks` で足り、新規に要るのは
**画面単位の集約 GET**（`app/api/board.py`・`GET /board`）だけ。カラム振り分けは status からの導出で
フロントが行い、サーバーは `rank, id` 順のフラット列＋各タスクの `_etag` を返す（N+1 にしない・二重の
真実を作らない — D-20）。412 発生後の文言は `messageForError` に一本化し、既存バックログも載せ替えて
一貫適用（D-24）。E2E フロー③の `test.fixme` を外した。直前は B-22 プランニングモード（右ペイン）を
完了。M3 は 3/4（残り B-14 任意）、M4 は 6/6 達成、M5 は 3/6（B-21・B-22・B-23 完了・次は B-24 進捗
表示、または B-26 楽観排他 UX の実装〈方針 D-24 は決定済み〉）。B-series 22/33＋EX 2/2）_
