...
_最終更新: 2026-08-03（B-19 PBI の分割を完了し、**M4 が 6/6 で達成**。分割を「分割元を指す
新規作成」として実装した——生成物は状態 `new` から始まる通常の PBI で、唯一 `parentPbiId` に
分割元を刻む。分割元は変更しないため作成であって更新ではなく、`If-Match` を要さない
（`POST .../{pbi_id}/split`・データ層 `split_pbi`）。子は末尾に採番し（同一 rank 隣接への
`rank_between` 衝突を避ける）、位置ではなく参照で辿らせる。フロントはバックログ画面に「分割」
ボタンと「↳ 分割元」リンク（集約 GET が既に載せる `parentPbiId` を利用）を足した。
直前は EX-2 カバレッジ PR コメントの刷新を完了（従来の「絶対値だけ」を、受け手が行動できる 2 つの
事実——**base(main) からの Δ**と**未カバーの変更行の名指し**——に置き換え。数値ゲートは D-19 が
却下したため置かない。[`D-23`](./decisions/D-23-coverage-comment.md)）。
M3 は 3/4（残り B-14 任意）、M4 は 6/6 達成。次は M5（B-21〜）。B-series 19/33＋EX 2/2）_
