...
| **M5** | ★**スプリントが1周回る**（ここからドッグフーディング） | B-21 〜 B-26 | 🟨 4 / 6（B-21・B-22・B-23・B-24 完了） |
...
_最終更新: 2026-08-06（B-24 進捗表示を完了。着手前に「営業日」の定義を
[`D-25`](./decisions/D-25-business-days-and-timezone.md) で決めた——土日に加えて**日本の祝日も除外**
（`jpholiday` に委譲）・「今日」は **Asia/Tokyo** の暦日で数え、注入可能にする（P-1/P-2）。進捗は
提案書 05章の**2本バー**（計画タスク / チームタスク）＋営業日マーカー。要点は **B-23 が用意した
join point に足すだけ**で、集計は `GET /board` が既に引いた `tasks` から数え追加クエリを撃たない
（N+1 を作らない）。集計は**データ層の純関数** `app/data/progress.py`（`compute_progress`・`today`
引数注入）に一元化し、API 層は `BoardResponse.progress` を足すだけ。フロントは共通目盛で2本を並べ
（長さがそのまま件数比）、マーカーを経過/総営業日の位置に置く。**色は種別の区別のみで警告色は使わない**
（P-1 / D-13）。期間未設定ならマーカーは描かない（P-1）。直前は B-23 スプリント画面のボードを完了。
M3 は 3/4（残り B-14 任意）、M4 は 6/6 達成、M5 は 4/6（B-21・B-22・B-23・B-24 完了・次は B-25
スプリント終了処理〈I-5 の担保〉、または B-26 楽観排他 UX〈方針 D-24 は決定済み・B-23 で実装済み〉）。
B-series 23/33＋EX 2/2）_
