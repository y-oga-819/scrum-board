...
| **M4** | プロダクトバックログが運用できる | B-15 〜 B-20 | 🟨 5 / 6（B-15・B-16・B-17・B-18・B-20 完了・B-19 未着手。E2E①の緑化は横断課題として B-11 に集約） |
...
- [ ] Playwright で主要フロー5本が**通る**　→ 受け皿5本は `frontend/e2e/`（`test.fixme`）。各フローの**画面导线・本体**は担当PBIで用意する（①B-17/B-20 ②B-22 ③B-23 ④B-25 ⑤B-29）。ただし**5本すべてに共通するサインイン経路**（`playwright.config.ts` の宿題。テスト用トークン注入 or 開発用サインイン省略 B-14 のいずれか）が未決で、これが解けるまで実際の緑化（`test.fixme` 解除）はできない。**この共通の緑化ゲートは B-11 が持つ**——各機能PBIはフロー本体まで用意すれば完了とし、`test.fixme` 解除は本項目で一括して閉じる
...
> 担当PBIの完了条件へ明示的に移した**。E2E フローは対象画面が揃う各PBI（①B-17/B-20 ②B-22
> ③B-23 ④B-25 ⑤B-29）で**フロー本体まで**用意する（ハーネスは完成済み）。ただし
> `test.fixme` の解除＝実際の緑化は**5本共通のサインイン経路**（`playwright.config.ts` の宿題・
> B-14 が一つの選択肢）に依存し、これは機能PBIごとに解けない横断課題のため **B-11 に集約**して
> ここで一括して閉じる（各機能PBIはフロー本体まで＝完了）。マージブロックは B-13 でブランチ
> 保護を有効化すれば閉じる（手順書あり）。加えて不変条件の振る舞い検査（`tests/invariants/`
> の skip）は検証関数を持つ **B-20** で有効化した（済）。
...
> 閉じられない横断課題（5本共通のサインイン経路）に依存するため、その責務は **B-11 に集約**
...
> - **E2E フロー①の緑化**（`test.fixme` 解除）は 5 本共通のサインイン経路に依存 → **B-11**（横断課題として集約）
...
- [x] **E2E フロー①の「PBI 作成」導線を用意する**（`PBI を追加`／`タイトル`／`保存`）　→ フロー①の本体は B-20 で書き上げ済み（`frontend/e2e/signin-pbi-task.spec.ts`）。`test.fixme` 解除＝実際の緑化は 5 本共通のサインイン経路に依存するため **B-11 に集約**（この PBI 単独では閉じられない横断課題。D-19 主要フロー網羅）
...
> 課題のため、責務を **B-11 に集約**した（B-20 の機能スコープは完結）。
...
- [x] **E2E フロー①の「タスク追加」導線とフロー本体を用意する**（`frontend/e2e/signin-pbi-task.spec.ts`）　→ タスク追加UI（`タスクを追加`／`タスク名`／`追加`）を載せ、サインイン→PBI作成→タスク追加のフロー本体を書き上げた。`test.fixme` 解除＝実際の緑化は 5 本共通のサインイン経路（`playwright.config.ts` の宿題・B-14 が一つの選択肢）に依存する横断課題のため **B-11 に集約**（D-19 主要フロー網羅）
...
- [ ] **完了タスクは移動しない・`sprintId` を変更しない**（未完了のみ次スプリントへ）　→ **不変条件 I-5 の担保はここ**（単一文書で表せない操作級の不変条件。B-20 の `check_invariants` は I-1〜I-4 の単一文書のみを見る）
- [ ] 強制も警告もせず事実だけ見せる（P-1）
...
_最終更新: 2026-08-01（B-20 タスクのCRUDを完了。タスクのドメイン規則と不変条件
I-1〜I-4 を単一関数 `check_invariants`（`app/data/tasks.py`）に集約し、B-11 のテーブル駆動
振る舞い検査を有効化。I-5 は操作をまたぐため B-25 の終了処理に閉じる。
`/api/products/{pid}/tasks` の CRUD（team タスクも作成可・不変条件違反は 422+violations）、
`GET /backlog` への配下タスク結合（N+1 なし）、バックログ画面のタスク表示・追加UIまで通した。
E2E フロー①は本体を書き上げ、緑化（`test.fixme` 解除）は 5 本共通のサインイン経路に依存する
横断課題として **B-11 に集約**（機能スコープは完結）。あわせて同じ理由で保留だった B-17 も
完了に。M4 は 5/6（残り B-19）。B-11 進行中）_
