...
| **M5** | ★**スプリントが1周回る**（ここからドッグフーディング） | B-21 〜 B-26 | 🟨 1 / 6（B-21 完了） |
...
_最終更新: 2026-08-03（B-21 スプリントの CRUD を完了し、**M5 に着手**。スプリントを PBI の CRUD
（B-15）と同型に実装した——ドメイン規則（状態遷移 `planned → active → closed` の一方向前進・
期間逆転の判定・`number` の連番採番）を純関数 `app/data/sprints.py` に一元化し、HTTP への翻訳
（422・`violations`・`ETag`・`If-Match` 必須）だけを `app/api/sprints.py` に置く。`number` と作成時
`status` は入力に取らず（採番はサーバー所有・始点は必ず `planned`）、一覧は各要素が `_etag` を
本文で運ぶ。同時 `active` を1つに絞る操作級の制約は「今どのスプリントか」を決める B-22/B-23 に
繰り延べた（I-5 を B-25 に閉じたのと同じ切り分け）。
直前は B-19 PBI の分割を完了し **M4 を 6/6 で達成**（分割を「分割元を指す新規作成」として実装。
生成物は状態 `new` の通常 PBI で唯一 `parentPbiId` を刻み、分割元は不変ゆえ `If-Match` 不要）。
M3 は 3/4（残り B-14 任意）、M4 は 6/6 達成、M5 は 1/6（B-21 完了・次は B-22 プランニング）。
B-series 20/33＋EX 2/2）_
