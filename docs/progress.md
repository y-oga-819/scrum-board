...
_最終更新: 2026-08-02（EX-2 カバレッジ PR コメントの刷新を完了。従来の「絶対値だけ」を、
受け手が行動できる 2 つの事実——**base(main) からの Δ**（元の状態→変化）と**未カバーの変更行の
名指し**（次の一手）——に置き換えた。数値ゲート（率の下限・Pass/Fail・ビルド失敗）は D-19 が
却下した「率に下限を設ける」に当たるため置かず、判断は書き手に返す（[`D-23`](./decisions/D-23-coverage-comment.md)）。
集計は stdlib のみの Python ツール `scripts/coverage/report.py`（`unittest` 検証・`make test-scripts`）に
集約し、base は外部SaaS/サードパーティ action に依存せず自前 artifact（直近 main run）から取る。
直前は EX-1 E2E 実行整備を完了（env ゲート resolver + e2e ビルドでサインインをバイパス・`webServer`
起動・`prd_test_<runId>` seeding／物理削除。[`D-22`](./decisions/D-22-e2e-execution.md)）。
M3 は 3/4（残り B-14 任意）、M4 は 5/6（残り B-19）。B-series 18/33＋EX 2/2）_
