...
| マイルストーン | 到達点 | PBI | 進捗 |
|:---:|:---|:---|:---:|
| **M1** | ★**1ページがEntra IDで保護される**（Easy Authなし） | B-03, B-01, B-05, B-06, B-02, B-04 | 0 / 6 |
| **M2** | ★**認可まで通る**（非メンバーは403・初回サインインで自動参加） | B-07, B-26, B-14, B-27 | 0 / 4 |
| **M3** | 開発の土台（テスト/CI/API規約/リポジトリ規約） | B-00, B-25, B-29, B-30 | 0 / 4 |
| **M4** | プロダクトバックログが運用できる | B-08〜B-13 | 0 / 6 |
| **M5** | ★**スプリントが1周回る**（ここからドッグフーディング） | B-15〜B-19, B-28 | 0 / 6 |
| **M6** | デイリースクラムがこの画面だけで完結する | B-20〜B-22 | 0 / 3 |
| **M7** | 実運用に耐える | B-23〜B-24 | 0 / 2 |
| **M8** | *（将来）* プロジェクトを自分たちで管理できる | B-31, B-32 | 0 / 2 |
| | | **合計** | **0 / 33** |
...
```
M1 認証PoC   B-03 骨格(1ページ) → B-01 Entra登録 → B-05/B-06 認証
                                → B-02/B-04 でデプロイして公開URLでも確認
M2 認可PoC   B-07 データ層 → B-26 マイグレーション → B-14 認可 → B-27 ブートストラップ
M3 土台      テスト基盤・API規約（スクラム機能を作り込む前に）
M4〜 アプリ本体
```

PoC自体を無検証で進めないため、**V-1〜V-4 のテストは B-06 の完了条件に含め**、
テスト基盤の全体構築（B-00）を待たない。
...
| [D-21](./decisions/D-21-bootstrap-and-migration.md) | マイグレーション・ユーザー登録・**認証の実装順序** | B-26, B-27, B-30 |
...
> ローカル（`localhost:4200`）で検証してから、B-02/B-04 でデプロイし公開URLでも確認する。

### ⬜ B-03 プロジェクトの骨格を作る　`依存: —`
- [ ] Angular のビルド成果物を FastAPI が配信する
- [ ] フロントとAPIが同一 App Service に同居する構成
- [ ] ローカルで1コマンド起動できる
- [ ] **画面が1ページ表示される**（これが認証をかける対象になる）
...
### ⬜ B-05 フロントエンドのサインイン　`依存: B-01, B-03`
- [ ] MSAL でサインインできる
- [ ] **未認証ユーザーはルートガードで弾かれる**（＝ページに制限がかかる）
- [ ] ローカルでも本番と同じ経路で動く（**Easy Auth に依存しない** — D-10）

### ⬜ B-06 APIのトークン検証　`依存: B-01, B-03`
...
- [ ] 改ざんトークンで 401 を返す
- [ ] **V-1〜V-4 のテストが書かれている**（テスト基盤の全体構築=B-00 を待たない）
...
### ⬜ B-27 ユーザー登録と認可ブートストラップ　`依存: B-14, B-26`
> **方針決定済み** → [`D-21`](./decisions/D-21-bootstrap-and-migration.md)
...
### ⬜ B-30 ゲストログイン経路　`依存: B-06, B-26`　🔸**任意**
> **方針決定済み** → [`D-21`](./decisions/D-21-bootstrap-and-migration.md)
...
- [ ] B-06のユーザー解決ポートに**実装をもう1つ足すだけ**で実現している
...
| **B-30** | 任意 | 開発時にサインインを省略する抜け道。当初の存在理由（画面開発の解禁）は認証を先に作る順序に変えたため消えた |
